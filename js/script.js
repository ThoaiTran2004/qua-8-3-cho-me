...
function createFallingMessages() {
    ...
    // Tạo 1 lời chúc mỗi lần để tránh chồng chéo
    const batchSize = 1; // Giảm xuống 1 để dễ đọc hơn
    let messageIndex = 0;
    let positionIndex = 0; // Theo dõi vị trí tuần tự
    ...
    function createBatch() {
        ...
        for (let i = 0; i < batchSize; i++) {
            setTimeout(() => {
                ...
                // Tạo vị trí đều đặn theo tuần tự
                const positions = [10, 25, 40, 55, 70, 85]; // 6 vị trí cố định, dày đặc hơn
                const xPos = positions[positionIndex % positions.length];
                positionIndex++; // Tăng index để lần sau dùng vị trí tiếp theo
                
                messageDiv.style.left = xPos + '%';
                messageDiv.style.animationDelay = '0s'; // Không có delay ngẫu nhiên
                messageDiv.style.fontSize = '20px'; // Tăng font-size từ 16px
                ...
                messageIndex++;
            }, i * 400); // Giảm xuống 400ms để lời chúc xuất hiện dày đặc hơn
        }
        
        // Lặp lại tạo batch mới - thường xuyên hơn
        setTimeout(createBatch, 1500); // Giảm xuống 1.5 giây để lời chúc xuất hiện dày đặc hơn
    }
    
    // Bắt đầu tạo batch đầu tiên
    createBatch();
    
    // Bắt đầu tạo hình ảnh mẹ rơi xuống
    createFallingMotherImages();
}
...
function createFallingMotherImages() {
    let container = document.getElementById('fallingMessages');
    if (!container) {
        // Tạo container nếu không tồn tại
        container = document.createElement('div');
        container.id = 'fallingMessages';
        ...
        console.log('✅ Created fallingMessages container for mother images');
    }
